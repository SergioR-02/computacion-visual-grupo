...
import { useGLTF, useFBX, useAnimations, Text, Box } from '@react-three/drei'

function AvatarScene({ selectedColor, animationName }) {
  const group = useRef()
  const [error, setError] = useState(null)
  const [isLoading, setIsLoading] = useState(true)
  const [modelType, setModelType] = useState('gltf') // 'gltf' or 'fbx' or 'primitive'

  // Try GLTF first, then FBX, then primitive fallback
  let model = null
  let actions = null
  ...
  try {
    if (modelType === 'gltf') {
      ...
      const { actions: gltfActions } = useAnimations(gltf.animations, group)
      model = gltf.scene
      actions = gltfActions
    }
  } catch (err) {
    console.error('GLTF loading failed:', err)
  }

  try {
    if (!model && modelType === 'fbx') {
      const fbx = useFBX('/Rumba_Dancing.fbx')
      const { actions: fbxActions } = useAnimations(fbx.animations, group)
      model = fbx
      actions = fbxActions
    }
  } catch (err) {
  ...
  useEffect(() => {
    if (model) {
      ...
      // Try fallback models or primitive
      if (modelType === 'gltf') {
        setModelType('fbx')
      } else if (modelType === 'fbx') {
        setModelType('primitive')
        setError('3D models not available, using primitive avatar')
        setIsLoading(false)
      }
    }
  }, [model, selectedColor, modelType])
  ...
  useEffect(() => {
    if (actions && Object.keys(actions).length > 0) {
      ...
      if (animationName && actions[animationName]) {
        ...
        // If specific animation not found, play the first available
